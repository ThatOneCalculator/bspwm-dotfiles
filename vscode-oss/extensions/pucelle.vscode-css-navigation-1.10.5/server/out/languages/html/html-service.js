...
(function (HTMLService) {
    ...
    async function getImportPathAt(document, position) {
        let offset = document.offsetAt(position);
        let importPath = new html_scanner_1.HTMLScanner(document, offset).scanForImportPath();
        if (!importPath && isJSXDocument(document)) {
            importPath = new jsx_scanner_1.JSXScanner(document, offset).scanForImportPath();
        }
        if (importPath) {
    ...
    async function scanStyleImportPaths(document) {
        ...
        let importFilePaths = [];
        while (match = re.exec(text)) {
            let relativePath = utils_1.firstMatch(match[0], hrefRE);
            if (!relativePath) {
                continue;
            }
            let filePath = await file_1.resolveImportPath(documentPath, relativePath);
            if (filePath) {
